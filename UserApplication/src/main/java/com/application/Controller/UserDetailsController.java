...
public class UserDetailsController {
	...
	public String saveuser(@ModelAttribute("userinfo") UserDetails userinfo, ModelMap map) {

		UserDetails fetchuser = userService.findByEmail(userinfo.getEmail());
		...
		if (fetchuser != null) {
			...
			UserDetails saveuser = userService.save(userinfo);
	...
	public String userlogin(@ModelAttribute UserLogin login, ModelMap map) {
		...
		UserDetails userinfo = userService.findByEmail(login.getEmail());
		if (userinfo != null) {
			...
			if (userinfo.getPassword().equals(login.getPassword())) {
				...
				if (userinfo.getRole().equals(Role.USER)) {
					...
					Iterable<UserDetails> listusers = userService.findAll();
	...
	public String updateuser(@ModelAttribute("userdetails") UserDetails userdetails, ModelMap map) {

		UserDetails userinfo = userService.findById(userdetails.getId());
//		UserDetails userfecth = userService.findByEmail(userinfo.getEmail());
	...
	public String admindetails(ModelMap map, @PathVariable("id") long id) {
		UserDetails fetchadmin = userService.findById(id);
	...
	public String uploadfile(ModelMap map, @PathVariable("id") long id, @RequestParam("file") MultipartFile file)
		...
		if (file.isEmpty() || file == null || file.getOriginalFilename() == null) {
			...
			System.out.println(fetchfile.size());
			userdata.setAvailable_files(fetchfile.size());
			UserDetails userdetails = userService.save(userdata);
	...
	public String deletefile(ModelMap map, @PathVariable("id") long id) {
		...
		UserDetails fetchuser = userService.findById(file.getUserid().getId());
		...
		UserDetails saveuser = userService.save(fetchuser);
		map.addAttribute("user", saveuser);
